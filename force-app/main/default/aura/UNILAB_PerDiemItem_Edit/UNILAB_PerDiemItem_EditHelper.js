({
    ...
    valueChange : function(component, event) {
        ...
        if(tempEvent.length>0) {
        	...
        	component.set("v.existEventData", tempEvent);
        }
    },
    eventDataHelper : function(component, selectedRow, selectedRowData, eventDataArr, exemptRemarks) {
        
        var existRemoveList = component.get("v.removeEventData");
        var removeList=[...existRemoveList];
        ...
        if(selectedRow) {
            ...
            if(eventDataArr.length > 0) {
                eventDataArr.map((resp, key) => {
                    if(resp.Id == selectedRowData.Id){
                    	eventDataArr.splice(key,1);
                    	removeList.push(resp);
                	}
                })
            }
        }
        ...
        component.set("v.existEventData", eventDataArr);
        component.set("v.removeEventData", removeList);
        
        //console.log(component.get("v.removeEventData"));
    }
})